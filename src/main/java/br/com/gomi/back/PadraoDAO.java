...
public abstract class PadraoDAO<T> extends PadraoViewModel {
	...
	protected abstract String[] CriaParametros(T model); // Talvez nao seja statement, mas aqui é para passar
															// parametros
	...
	public int insert(T model) throws Exception {
		JDataTable tab = HelperDAO.executaProcSelect("exec spInsert_" + getTabela() + getQtdParametros(),
				CriaParametros(model));
